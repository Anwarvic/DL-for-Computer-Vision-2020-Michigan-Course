...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i, train_point in enumerate(x_train):
    for j, test_point in enumerate(x_test):
      dists[i, j] = torch.sum((train_point - test_point)**2)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  x_test = x_test.view(num_test, -1)
  for i, train_point in enumerate(x_train):
    dists[i,:] = torch.sum((train_point - x_test)**2, axis=1)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  _, indices = torch.topk(dists, k, axis=0, largest=False)
  kn = y_train[indices]
  y_pred, _ = torch.mode(kn, dim=0)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return y_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
      ...
      for i in range(num_folds):
          ...
          curr_acc = classifier.check_accuracy(x_val, y_val, k)
          k_to_accuracies[k].append(curr_acc)
    #   k_to_accuracies[k] = acc / num_folds
